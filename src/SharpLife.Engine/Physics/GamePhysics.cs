...
namespace SharpLife.Engine.Physics
{
    ...
    public sealed class GamePhysics
    {
        ...
        //TODO: create
        private readonly IVariable<float> _sv_clienttrace;
        ...
        private void FindTouchedLeafs(Collider ent, BaseNode node, ref int topnode)
        {
            ...
            if (node.Contents < Contents.Node)
            {
                ...
                ent.AddLeafNumber((short)_worldModel.BSPFile.Leaves.IndexOf((Leaf)node));
                return;
            }

            var currentNode = (Node)node;

            var result = PhysicsUtils.BoxOnPlaneSide(ent.AbsMin, ent.AbsMax, currentNode.Plane);
        ...
        private Hull HullForBsp(Collider ent, in Vector3 mins, in Vector3 maxs, out Vector3 offset)
        {
            var model = ent.Renderable.Model;

            if (model == null)
            {
                throw new InvalidOperationException($"Hit a {ent.Entity.ClassName} with no model");
            }

            if (!(model is BSPModel bspModel))
            {
                throw new InvalidOperationException($"Hit a {ent.Entity.ClassName} with wrong model type ({model.GetType().Name}:{model.Name})");
            }
        ...
        private Contents LinkContents(AreaNode node, ref Vector3 pos)
        {
            foreach (var entity in node.Solids)
            {
                ...
                if (entity.GroupInfo != 0 && !TestGroupOperation(entity.GroupInfo, GroupMask))
                {
                    continue;
                }

                if (!(entity.Renderable is BSPRenderableComponent))
                {
                    continue;
                }
                ...
                {
                    _logger.Debug("Invalid contents on trigger field: {0}", entity.Entity.ClassName);
                }
        ...
        private void TouchLinks(Collider ent, AreaNode node)
        {
            foreach (var touched in node.Triggers)
            {
                ...
                if (touched.GroupInfo != 0
                    && ent.GroupInfo != 0
                    && !TestGroupOperation(touched.GroupInfo, ent.GroupInfo))
                {
                    continue;
                }
                ...
                if (touched.Renderable is BSPRenderableComponent)
                {
                    var hull = HullForBsp(touched, ent.Mins, ent.Maxs, out var offset);

                    var localPosition = ent.Transform.Origin - offset;
                ...
                touched.Entity.SendMessage(BuiltInComponentMethods.Touch, ent);
            }

            if (node.Axis != -1)
            {
                if (ent.AbsMax.Index(node.Axis) > node.Distance)
                {
                    TouchLinks(ent, node.Children[0]);
                }

                if (node.Distance > ent.AbsMin.Index(node.Axis))
                {
        ...
        public void UnlinkEdict(Collider ent)
        {
            if (ent.Area != null)
            {
                //TODO: optimize
                ent.Area.Triggers.Remove(ent);
                ent.Area.Solids.Remove(ent);
                ent.Area = null;
            }
        }

        public void LinkEdict(Collider ent, bool touchTriggers)
        {
            UnlinkEdict(ent);

            if (!ent.Entity.IsWorld() && !ent.Entity.Destroyed)
            {
                //TODO: should avoid doing this
                ent.Entity.SendMessage(BuiltInComponentMethods.SetAbsBox);

                //TODO: obsoleted by parenting system
                /*
                if (ent.MoveType == MoveType.Follow && ent.AimEntity != null)
                {
                    ent.CopyNodeStateFrom(ent.AimEntity);
                }
                else
                */
                {
                    ...
                    if (ent.Renderable.Model != null)
                    {
                        int topNode = -1;

                        FindTouchedLeafs(ent, _worldModel.BSPFile.Nodes[0], ref topNode);

                        if (ent.LeafCount > PhysicsConstants.MaxLeafs)
                        {
                            ent.MarkLeafCountOverflowed(topNode);
                        }
                    }
                }
                ...
                {
                    if (ent.Solid == Solid.BSP && ent.Renderable.Model == null)
                    {
                        _logger.Debug($"Inserted {ent.Entity.ClassName} with no model");
                        return;
                    }
                }
                ...
                while (i.Axis != -1)
                {
                    if (ent.AbsMin.Index(i.Axis) > i.Distance)
                    {
                        i = i.Children[0];
                    }
                    else if (ent.AbsMax.Index(i.Axis) < i.Distance)
                    {
                        i = i.Children[1];
                    }
                    else
        ...
        private Hull[] HullForEntity(Collider ent, in Vector3 mins, in Vector3 maxs, out Vector3 offset)
        {
        ...
        private Hull[] HullForStudioModel(Collider collider, StudioRenderableComponent studioRenderable, in Vector3 mins, in Vector3 maxs, out Vector3 offset, out int pNumHulls)
        {
            var size = maxs - mins;
            ...
            else if ((studioRenderable.Entity.Flags & EntityFlags.Client) == 0)
            {
                useStudioHull = true;
                sizeScale = 0.5f;
            }
            else if (_sv_clienttrace.Value == 0)
            {
                useStudioHull = false;
                sizeScale = 0.5f;
            }
            ...
            {
                sizeScale = _sv_clienttrace.Value * 0.5f;
                useStudioHull = true;
                size.X = size.Y = size.Z = 1;
            }

            var studioModel = studioRenderable.StudioModel;

            //TODO: rework
            var skipShield = false;// !pEdict.HasShield && false;// (g_bIsTerrorStrike || g_bIsCStrike || g_bIsCZero);
            ...
            if (useStudioHull || (studioModel.StudioFile.Flags & MDLFlags.ComplexModelIntersection) != 0)
            {
                ...
                if ((studioRenderable.Entity.Flags & EntityFlags.Client) != 0)
                {
                    var angles = collider.Transform.Angles;

                    StudioPlayerBlend(studioModel.StudioFile.Sequences[(int)studioRenderable.Sequence], out var iBlend, ref angles.X);

                    _studioHullBlenders[0] = (byte)iBlend;

                    return _studioCache.StudioHull(
                        studioModel,
                        studioRenderable.Frame,
                        (int)studioRenderable.Sequence,
                        angles,
                        collider.Transform.Origin,
                        size,
                ...
                else
                {
                    return _studioCache.StudioHull(
                        ...
                        studioRenderable.Frame,
                        (int)studioRenderable.Sequence,
                        collider.Transform.Angles,
                        collider.Transform.Origin,
                        size,
                        studioRenderable.Controllers,
                        studioRenderable.Blenders,
                        out pNumHulls,
                        skipShield);
                }
            }
            ...
            pNumHulls = 1;
            return HullForEntity(collider, mins, maxs, out offset);
        }
        ...
        private void SingleClipMoveToEntity(Collider ent, in Vector3 start, in Vector3 mins, in Vector3 maxs, in Vector3 end, out Trace trace)
        {
            trace = new Trace
            {
                Fraction = 1.0f,
            ...
            //TODO: let renderable return hulls
            if (ent.Renderable is StudioRenderableComponent model)
            {
                pHulls = HullForStudioModel(ent, model, mins, maxs, out offset, out numhulls);
            }
            else
            {
            ...
            var rotated = ent.Solid == Solid.BSP && (transform.Angles.X != 0 || transform.Angles.Y != 0 || transform.Angles.Z != 0);

            if (rotated)
            {
                VectorUtils.AngleToVectors(transform.Angles, out up, out right, out forward);
            ...
            if (trace.Fraction != 1.0)
            {
                if (rotated)
                {
                    VectorUtils.AngleToVectorsTranspose(ent.Transform.Angles, out forward, out right, out up);
        ...
        private void MoveBounds(Vector3 start, ref Vector3 mins, ref Vector3 maxs, ref Vector3 end, out Vector3 boxmins, out Vector3 boxmaxs)
        {
            boxmins = new Vector3();
            boxmaxs = new Vector3();
        ...
        private bool CheckSphereIntersection(Collider ent, in Vector3 start, in Vector3 end)
        {
            if ((ent.Entity.Flags & EntityFlags.Client) == 0)
            {
                return false;
            }

            //TODO: error handling
            var animating = ent.Renderable as StudioRenderableComponent;

            var pModel = animating.Model as StudioModel;
            ...
            return DoesSphereIntersect(ent.Transform.Origin, fSphereRadiusSquared, traceOrg, traceDir);
        }
        ...
        private void ClipToLinks(AreaNode node, ref MoveClip clip)
        {
            ...
            {
                if (pEntity.GroupInfo != 0 && clip.PassEntity?.GroupInfo != 0
                    && !TestGroupOperation(pEntity.GroupInfo, clip.PassEntity.GroupInfo))
                {
                    continue;
                }
                ...
                */
                ...
                {
                    if ((pEntity.Entity.Flags & EntityFlags.MonsterClip) != 0 && !clip.MonsterClipBrush)
                    {
                        continue;
                    }
                }
                ...
                if (clip.IgnoreTransparent && pEntity.Entity.GetRenderMode() != RenderMode.Normal && (pEntity.Entity.Flags & EntityFlags.WorldBrush) == 0)
                {
                    continue;
                }
                ...
                if (clip.PassEntity != null
                    && (ReferenceEquals(pEntity.Owner, clip.PassEntity)
                    || ReferenceEquals(clip.PassEntity.Owner, pEntity)))
                {
                    continue;
                }
                ...
                if ((pEntity.Entity.Flags & EntityFlags.Monster) != 0)
                {
        ...
        public Trace Move(in Vector3 start, in Vector3 mins, in Vector3 maxs, in Vector3 end, TraceType type, Collider passedict, bool ignoreTransparent, bool monsterClipBrush)
        {
            var clip = new MoveClip();

            SingleClipMoveToEntity(_scene.World, start, mins, maxs, end, out clip.Trace);
            ...
            MoveBounds(start, ref clip.Mins2, ref clip.Maxs2, ref clip.End, out clip.BoxMins, out clip.BoxMaxs);
